...
def extract_ssa_national_directory(path: str | Path) -> Iterator[tuple[str, str, str]]:
    """Yield (name, name_type, source_field) from extracted SSA national yob*.txt files."""
    for member in sorted(Path(path).glob("yob*.txt")):
        with member.open("r", encoding="utf-8", newline="") as fh:
...
def extract_ssa_national_path(path: str | Path) -> Iterator[tuple[str, str, str]]:
    """Yield SSA national names from either the official ZIP or an extracted directory."""
    path = Path(path)
    if path.is_dir():
        yield from extract_ssa_national_directory(path)
    else:
        yield from extract_ssa_national_zip(path)
