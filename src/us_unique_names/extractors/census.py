...
def extract_census_surname_zip(zip_path: str | Path, name_type: str = "last") -> Iterator[tuple[str, str, str]]:
    ...
    When a ZIP contains the same table in multiple formats, the CSV member is preferred.
    """
    with zipfile.ZipFile(zip_path) as zf:
        members = sorted(zf.namelist())
        csv_members = [member for member in members if member.lower().endswith(".csv")]
        excel_members = [member for member in members if member.lower().endswith((".xlsx", ".xls"))]
        for member in [*(csv_members[:1]), *(excel_members[:0] if csv_members else excel_members[:1])]:
            lower = member.lower()
