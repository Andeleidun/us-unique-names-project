...
def validate_candidate_name(name: str, name_type: str) -> ValidationResult:
    ...
    lowered_tokens = {t.casefold().rstrip(".") for t in _tokens(name)}
    forbidden = lowered_tokens.intersection(DISALLOWED_EXACT_TOKENS)
    if forbidden:
...
def validate_public_rows(rows: Iterable[str], name_type: str) -> ValidationResult:
    result = ValidationResult(valid=True)
    seen: set[str] = set()
    ...
    for row_number, name in enumerate(rows, start=2):
        ...
        row_result = validate_candidate_name(normalized.display, name_type)
